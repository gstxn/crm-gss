...
const NovoMedico = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [especialidadesRes, estadosRes] = await Promise.all([
        ...
        setError('Não foi possível carregar os dados iniciais. Por favor, tente novamente.');
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('/api/medicos', formData);
      navigate(`/medicos/${response.data._id}`);
    } catch (err) {
      ...
      if (err.response && err.response.data && err.response.data.message) {
        ...
        setError('Ocorreu um erro ao criar o médico. Por favor, tente novamente.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="novo-medico-container">
      ...
      <form onSubmit={handleSubmit} className="medico-form">
        ...
        <div className="form-actions">
          <Link to="/medicos" className="btn-cancelar">
            <FaTimes /> Cancelar
          </Link>
